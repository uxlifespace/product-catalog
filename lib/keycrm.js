...
function buildShipping({ deliveryMethod, cityName, fullName, phone, street, house, apartment, branchRef, branchName, deliveryAddress }) {
  ...
  if (deliveryMethod === 'courier') {
    ...
    // KeyCRM вимагає delivery_service_id разом з warehouse_ref (інакше 422).
    // Якщо ID служби доставки не налаштований через env — не передаємо warehouse_ref,
    // а лише текстову адресу відділення, щоб синхронізація не падала.
    const deliveryServiceId = process.env.KEYCRM_NP_DELIVERY_SERVICE_ID;
    if (branchRef && deliveryServiceId) {
      shipping.warehouse_ref = branchRef;
      shipping.delivery_service_id = Number(deliveryServiceId);
    }
    if (deliveryAddress) shipping.shipping_receive_point = deliveryAddress;
    else if (branchName) shipping.shipping_receive_point = branchName;
  }
  return shipping;
}
...
async function markOrderPaidInKeyCrm(keycrmOrderId, amount) {
  const r = await fetch(`${KEYCRM_BASE}/order/${keycrmOrderId}/payment`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({
  ...
  if (!r.ok) {
    const err = new Error('Помилка позначення оплати в KeyCRM: ' + JSON.stringify(data));
    err.details = data;
    throw err;
  }
  return data;
}
module.exports.markOrderPaidInKeyCrm = markOrderPaidInKeyCrm;
